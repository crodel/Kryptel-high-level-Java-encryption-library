...
final class Extractor4 implements ISilverKeyExtractor {
	Extractor4(long capabilities) throws Exception {
  ...
  //
	...
	public void ExtractData(String targetDir, String parcelPath, Object arg, IKeyCallback keyFunc, IProgressCallback progressFunc, IReplaceCallback replaceCallback, IMessage msgCallback, INotification notificationCallback) throws Exception {
		...
		try {
			...
			if (!locator.guidEngine.equals(CID_SILVER_KEY_4)) throw new Exception(Message.Get(Message.Code.WrongExtractor));
			if (locator.versionRequired > Engine4.ENGINE_VERSION) throw new Exception(Message.Get(Message.Code.OldVersion));
			if (locator.versionCreated < Engine4.MINIMAL_COMPATIBLE_ENGINE_VERSION) throw new Exception(Message.Get(Message.Code.IncompatibleVersion));
			...
			hmacSetup.SetBase((locator.versionRequired >= Engine4.ENGINE_VERSION_WITH_CORRECT_HMAC) ? CID_HASH_SHA512 : CID_HASH_SHA512_64);
